...
def on_event(camera, event_type, event_status, renderer):
    ...
    if event_type == SeekCameraManagerEvent.CONNECT:
        ...
        palette_data = SeekCameraColorPalette.USER_0
        palette_data = SeekCameraColorPaletteData()     

        #palette_data = SeekCameraColorPaletteData()

        for i in range(256):
            palette_data[i] = (i, i, i, i)

        palette_data[255] = (0, 0, 255, 150)
        camera.color_palette = SeekCameraColorPalette.USER_0 #
        ...
        camera.linear_agc_lock_max = 9000
...
def main():
    window_name = "nicoz Thermal seekcamera 1.0"
                ...
                if renderer.frame_condition.wait(150.0 / 1000.0):
                    ...
                    if renderer.first_frame:
                        ...
                        sharpen_filter=np.array([[-1,-1,-1],
                                                 [-1,9,-1],
                                                 [-1,-1,-1]])
                        sharped=cv.filter2D(img,-1000,sharpen_filter)
                    # Render the image to the window.
                    cv2.imshow(window_name, sharped)
